...
void test( int fp1, int fp2 )
{
   ...
   // auto mfem_mesh = mfem::Mesh::MakeCartesian2D( nx, ny, mfem::Element::Type::QUADRILATERAL, nx, ny, false );
   mfem::Mesh mfem_mesh = mesh_2d_orientation(fp1, fp2);
   ...
   dofs_in.Randomize(1);
   // dofs_in = 0.0;
   // const int dof_index = 1;
   // dofs_in[dof_index] = 1.0;
   // dofs_in = 1.0;
...
int main(int argc, char *argv[])
{
   ...
   const int n_perms = 4;
   for (int fp2=0; fp2<n_perms; ++fp2)
   {
      for (int fp1=0; fp1<n_perms; ++fp1)
      {
         std::cout << "\nTEST< " << fp1 << ", " << fp2 << " >:\n";
