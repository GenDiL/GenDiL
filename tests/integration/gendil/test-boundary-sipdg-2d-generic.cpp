...
int TestBoundarySIPDG()
{
   std::cout << "\n=== Boundary SIPDG test, order = " << order << " ===\n";
   ...
   // --------------------------------------------------------------------------
   // Generic boundary facet SIPDG weak form
   // --------------------------------------------------------------------------
   ...
   // Penalty parameters for Nitsche/SIPDG
   const Real tau_value = 10.0 * (order + 1) * (order + 1) / h;
   const Real sigma = 1.0; // symmetric interior penalty
   ...
   // Boundary SIPDG weak form:
   // - mu * dot(grad(u), Normal{}) * v  (consistency)
   // + tau * mu * u * v                 (penalty)
   // + sigma * u * mu * dot(grad(v), Normal{})  (adjoint consistency for symmetry)
   auto boundary_sipdg_wf =
      integrate(
         boundary_facets,
   ...
   std::cout << "PASS: Boundary SIPDG test for order " << order << "\n";
   return 0;
}
...
{
   int status = 0;
   status |= TestBoundarySIPDG<1>();
   status |= TestBoundarySIPDG<2>();
   status |= TestBoundarySIPDG<3>();
   return status;
}
