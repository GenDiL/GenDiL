   ...
   for ( GlobalIndex element = 0; element < num_elements; ++element )
   {
      UnitLoop< ComponentDofShape >( [&] ( auto... k )
      {
         ...
         x_data[fe_index] =
            ...
            0.25 * static_cast< Real >( local_index + 1 ) +
            static_cast< Real >( fe_index % 7 );
      });
   }
}
...
int TestVectorCellMassBsrAction()
{
   std::cout << "\n=== Vector cell mass BSR action test, order = "
             << order << " ===\n";

   constexpr Integer Dim = 3;
   const Integer n = 2;
   const Real h = 1.0 / n;
   CartesianMesh< Dim > mesh( { n, n, n }, { h, h, h }, { 0.0, 0.0, 0.0 } );
   ...
   {
      std::cerr
         << "FAILED: vector cell mass SGBSR action does not match "
         << "matrix-free action.\n";
      return 1;
   }
...
int main()
{
   if ( TestVectorCellMassBsrAction< 2 >() != 0 )
   {
      return 1;
   }

   std::cout << "\nVector cell mass BSR action test passed.\n";
   return 0;
}
