...
// Test diffusion operator with SYMMETRY TERM ONLY
// This isolates the symmetry face integral: ∫_Γ σ [[u]] {{μ ∇v · n}} ds
// No volume term, no consistency term, no penalty term
...
int TestDiffusionSymmetryOnly()
{
   std::cout << "\n=== Diffusion SYMMETRY TERM ONLY test, order = "
             << order << " ===\n";
   ...
   // --------------------------------------------------------------------------
   // Generic SYMMETRY TERM ONLY weak form
   // --------------------------------------------------------------------------
   ...
   {
      std::cerr << "FAILED: generic symmetry-only diffusion does not match assembled matrix.\n";
      std::cerr << "\nv_h generic: " << v_h_generic << "\n";
      std::cerr << "\nv_h assembled:  " << v_h_matrix << "\n";
      return 1;
   }

   std::cout << "SUCCESS: symmetry-only test, order " << order << " passed.\n";
   return 0;
}
...
int main()
{
   std::cout << "Testing diffusion operator with SYMMETRY TERM ONLY\n";
   std::cout << "This validates: ∫_Γ σ [[u]] {{μ ∇v · n}} ds\n";
   std::cout << "This is the term most likely to have issues with test gradient handling.\n\n";

   if (TestDiffusionSymmetryOnly<1>() != 0) { return 1; }
   if (TestDiffusionSymmetryOnly<2>() != 0) { return 1; }
   if (TestDiffusionSymmetryOnly<3>() != 0) { return 1; }

   std::cout << "\nAll symmetry-only diffusion tests passed.\n";
   return 0;
}
