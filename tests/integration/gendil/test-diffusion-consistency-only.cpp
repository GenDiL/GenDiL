...
// Test diffusion operator with CONSISTENCY TERM ONLY
// This isolates the consistency face integral: -∫_Γ {{μ ∇u · n}} [[v]] ds
// No volume term, no symmetry term, no penalty term
...
int TestDiffusionConsistencyOnly()
{
   std::cout << "\n=== Diffusion CONSISTENCY TERM ONLY test, order = "
             << order << " ===\n";
   ...
   // --------------------------------------------------------------------------
   // Generic CONSISTENCY TERM ONLY weak form
   // --------------------------------------------------------------------------
   ...
   {
      std::cerr << "FAILED: generic consistency-only diffusion does not match assembled matrix.\n";
      std::cerr << "\nv_h generic: " << v_h_generic << "\n";
      std::cerr << "\nv_h assembled:  " << v_h_matrix << "\n";
      return 1;
   }

   std::cout << "SUCCESS: consistency-only test, order " << order << " passed.\n";
   return 0;
}
...
int main()
{
   std::cout << "Testing diffusion operator with CONSISTENCY TERM ONLY\n";
   std::cout << "This validates: -∫_Γ {{μ ∇u · n}} [[v]] ds\n";
   std::cout << "This tests trial gradient averaging and normal computation.\n\n";

   if (TestDiffusionConsistencyOnly<1>() != 0) { return 1; }
   if (TestDiffusionConsistencyOnly<2>() != 0) { return 1; }
   if (TestDiffusionConsistencyOnly<3>() != 0) { return 1; }

   std::cout << "\nAll consistency-only diffusion tests passed.\n";
   return 0;
}
