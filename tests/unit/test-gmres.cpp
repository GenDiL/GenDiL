...
static bool almost_equal(Real x, Real y, Real tol = Real(1e-8))
{
...
bool test_small_2x2()
{
    ...
    struct A2x2 {
        void operator()(const Vector &v, Vector &w) const {
            w[0] = Real(2.0)*v[0] + Real(1.0)*v[1];
            w[1] = Real(1.0)*v[0] + Real(3.0)*v[1];
        }
    } A_mat;
    ...
    x = Real(0.0);
    b = Real(0.0);
    b[0] = Real(1.0); b[1] = Real(2.0);
    Vector x_expected(2);
    x_expected = Real(0.0);
    x_expected[0] = Real(0.2);  // 1/5
    x_expected[1] = Real(0.6);  // 3/5
...
bool test_random_spd_vs_cg()
{
    ...
    for(int i=0;i<(int)n;i++){
        for(int j=0;j<(int)n;j++){
            ...
            A(i,j) = sum + (i==j ? alpha : Real(0.0));
        }
    }
    Vector b(n); b = Real(0.0);
    for(int i=0;i<(int)n;i++){
        b[i] = dist(rng);
    }
    ...
    x_cg = Real(0.0); tmp = Real(0.0); z = Real(0.0); residual = Real(0.0); p = Real(0.0);
    const Integer cg_max = 1000;
    const Real cg_tol = Real(1e-12);
    auto dot = [&](const Vector &u, const Vector &v){ return Dot(u,v); };
...
bool test_poisson_vs_cg()
{
    ...
    auto coeff = [] GENDIL_HOST_DEVICE (const array<Real,Dim> &X){ return Real(1.0); };
    ...
    x0 = Real(0.0); tmp = Real(0.0); z = Real(0.0); residual = Real(0.0); p = Real(0.0);
    const Integer cg_max = 2000;
    const Real cg_tol = Real(1e-10);
    auto dot = [&](const Vector &u, const Vector &v){ return Dot(u,v); };
    auto [cg_conv, cg_iters, cg_relres] = ConjugateGradient(
    ...
    Vector x1(ndofs); x1 = Real(0.0);
    const Integer gm_max = 2000;
    const Integer restart = ndofs;
    const Real    gm_tol = Real(1e-10);
    ...
    if (!almost_equal(err_cg, err_gm, Real(1e-8))) pass = false;
...
bool test_happy_breakdown()
{
    ...
    struct A3x3 {
        void operator()(const Vector &v, Vector &w) const {
            w[0] = Real(5.0) * v[0];
            w[1] = Real(2.0) * v[1] + Real(1.0) * v[2];
            w[2] = Real(1.0) * v[1] + Real(3.0) * v[2];
        }
    } Aop;

    Vector v0(3), b(3);
    v0 = Real(0.0); v0[0] = Real(1.0);  // e0
    b = Real(0.0);
    Aop(v0, b);            // b = [5, 0, 0]
