...
void compare_solvers(const Integer n)
{
    ...
    auto sigma_mass = [] GENDIL_HOST_DEVICE (const array<Real,Dim> &X) { return Real(1.0); };
    auto mass_op = MakeMassFiniteElementOperator<KernelPolicy>(fe_space, int_rules, sigma_mass);

    const Real sigma_diff = Real(-1.0);
    const Real kappa = Real((order + 1) * (order + 1));
    auto velocity = [] GENDIL_HOST_DEVICE (const array<Real,Dim> &X) { return Real(1.0); };
    ...
    auto run_solver = [&](const char * name, auto && op, const Vector & b_rhs)
    {
        ...
        // Conjugate Gradient
        {
            ...
            x = Real(0.0); tmp = Real(0.0); z = Real(0.0); r = Real(0.0); pvec = Real(0.0);
        ...
        // GMRES
        {
            ...
            std::vector<Real> H((restart+1)*restart, Real(0.0));
            std::vector<Real> cs(restart, Real(0.0)), sn(restart, Real(0.0));
            std::vector<Real> e1_rhs(restart+1, Real(0.0)), y(restart, Real(0.0));
