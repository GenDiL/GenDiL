...
void test_mass_6D( const Integer n )
{
...
void test_range()
{
    ...
    while ( true )
    {
        Integer ndofs = Pow<6>(order+1) * Pow<6>(n);
        if ( ndofs > max_dofs ) break;

        test_mass_6D<order,num_quad_1d>(n);
        n *= 2;
    }
    cout << "       };\n";
...
int main()
{
    ...
    cout << "\n6D Mass‐Matrix Convergence Study\n"
         ...
         << "    \\begin{axis}[\n"
         << "       title={L2‐Error vs DoFs (6D Mass)},\n"
