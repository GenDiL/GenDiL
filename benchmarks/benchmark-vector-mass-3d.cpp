...
int main(int argc, char *argv[])
{
   ...
   // Finite element space
   auto struct_fe_space = MakeFiniteElementSpace( struct_mesh, vector_finite_element );
   auto cart3d_fe_space = MakeFiniteElementSpace( cart3d_mesh, vector_finite_element );
#ifdef GENDIL_USE_MFEM
   auto unstruct_fe_space = MakeFiniteElementSpace( unstruct_mesh, vector_finite_element );
#endif
   ...
   mfem::FiniteElementSpace mfem_fespace(&mfem_unstructured_mesh, &fec, vdim);
   mfem::BilinearForm mfem_operator(&mfem_fespace);
   mfem_operator.SetAssemblyLevel(mfem::AssemblyLevel::PARTIAL);
   mfem_operator.AddDomainIntegrator(new mfem::VectorMassIntegrator);
   ...
   }
   {
      ...
      const Integer num_elem_dofs = finite_element.GetNumDofs();
      const Integer num_elem = unstruct_fe_space.GetNumberOfFiniteElements();
      const Integer num_dofs = vdim * num_elem * num_elem_dofs;
      std::cout << "Dofs per element: " << num_elem_dofs << "\n Number of elements: " << num_elem << "\n";
      dofs_in = 1.0;
