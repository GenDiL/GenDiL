...
void ReferenceToNativeIndexInto(
   const std::array< Integer, Dim > & reference,
   const std::array< size_t, Dim > & sizes,
   const Permutation< Rank > & orientation,
   std::array< Integer, Dim > & native )
{
   ...
   gendil::ConstexprLoop< Rank >( [&] ( auto native_axis )
   {
      ...
      native[ Offset + native_axis ] = o > 0
         ? reference[ Offset + reference_axis ]
         : static_cast< Integer >(
   ...
   std::array< Integer, Dim > & native )
{
   ...
   {
      native[ Offset + axis ] = reference[ Offset + axis ];
   } );
}
   ...
   std::array< Integer, Dim > & native )
{
      ...
      TensorProductOrientation< Orientations... > > )
   {
            ...
            ++local_native_axis )
      {
         ...
         native[ native_axis ] = mapped_axis > 0
            ? reference[ reference_axis ]
            : static_cast< Integer >(
                 sizes[ native_axis ] - 1 - reference[ reference_axis ] );
      }
   }
   else
   {
      ...
      gendil::ConstexprLoop< sizeof...( Orientations ) >( [&] ( auto component )
      {
         ...
         ReferenceToNativeIndexInto< ComponentOffset >(
            reference,
            sizes,
...
/** @brief Convert a flat-oriented reference index to native axis order. */
...
GENDIL_HOST_DEVICE GENDIL_INLINE
constexpr std::array< Integer, Dim > ReferenceToNativeIndex(
   const std::array< Integer, Dim > & reference,
   const std::array< size_t, Dim > &,
   const IdentityOrientation< Rank > & )
{
...
/** @brief Convert a structured reference index directly to native axis order. */
   ...
   const TensorProductOrientation< Orientations... > & orientation )
{
   ...
   std::array< Integer, Dim > native{};
   detail::ReferenceToNativeIndexInto< 0 >(
      reference,
      sizes,
      orientation,
      native );
