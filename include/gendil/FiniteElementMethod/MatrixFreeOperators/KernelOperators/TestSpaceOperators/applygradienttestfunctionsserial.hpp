   ...
   Output & dofs_out )
{
   ...
   {
      // Face interpolation now works with ApplyGradientTestFunctionsImpl directly
      // after fixing the Rank calculation to use get_rank_v<> instead of InputTensor::Dim
      if constexpr ( Add )
   ...
   {
      // Volume interpolation uses ApplyGradientTestFunctionsImpl
      // which sums over all spatial dimensions internally
      if constexpr ( Add )
         dofs_out += details::ApplyGradientTestFunctionsImpl< 0 >( element_quad_data, DGuq );
      else
         dofs_out  = details::ApplyGradientTestFunctionsImpl< 0 >( element_quad_data, DGuq );
