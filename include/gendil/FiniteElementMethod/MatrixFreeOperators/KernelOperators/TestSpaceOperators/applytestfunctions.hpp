   ...
   std::index_sequence< I... > )
{
   return std::make_tuple(
      ApplyTestFunctions<DiffDim>(
         thread,
         GetVectorComponent<I>(quad_data),
         std::get<I>(quad_point_values))...);
}
   ...
   const std::tuple< InputTensors ... > & quad_point_values )
{
   static_assert(
      ...
      "ApplyTestFunctions requires one quadrature-value tensor per "
      "VectorDofToQuad component.");
   return ApplyTestFunctions<DiffDim>(
      thread,
      quad_data,
      quad_point_values,
 ...
 *   face_quad_data[local_face_index] = VectorDofToQuad<comp0, comp1>
 *
 ...
 * @param this_face_data VectorDofToQuad data for this face.
...
auto ApplyTestFunctions_ComponentWise(
   KernelContext & ctx,
   const VectorDofToQuad<ScalarMaps...>& this_face_data,
   const std::tuple< InputTensors... > & quad_point_values,
   std::index_sequence< I... > )
{
   // Static assert: number of components must match
   static_assert(
      sizeof...(InputTensors) == sizeof...(ScalarMaps),
   ...
   return std::make_tuple(
      ApplyTestFunctions(
         ctx,
         GetVectorComponent<I>(this_face_data),
         std::get<I>(quad_point_values))...
   );
}
 ...
 *     VectorDofToQuad<comp0, comp1>,  // face 0
 *     VectorDofToQuad<comp0, comp1>,  // face 1
 *     ...
 *   >
 *
 * This overload:
 ...
 *
 * @param quad_data Component-indexed VectorDofToQuad data.
   ...
   std::tuple< OutputTensors... > & dofs_out )
{
   ...
   static_assert(
      sizeof...(ScalarMaps) == sizeof...(InputTensors),
      "ApplyAddTestFunctions requires one input/output tensor per "
      "VectorDofToQuad component.");
   ...
   std::index_sequence< I... > )
{
   ...
   // For vector FE, face_quad_data is indexed FACE-FIRST:
   //   face_quad_data =
   //      tuple<VectorDofToQuad<comp0,comp1>, ...>
