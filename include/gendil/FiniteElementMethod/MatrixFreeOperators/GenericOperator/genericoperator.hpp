...
public:
   ...
   void operator()(const input& dofs_vector_in, output& dofs_vector_out) const
   {
      ...
      auto dofs_in = MakeReadOnlyEVectorView< KernelPolicy >( wf_ctx.template fe_field<TrialName>().space, dofs_vector_in );
      auto dofs_out = MakeWriteOnlyEVectorView< KernelPolicy >( wf_ctx.template fe_field<TestName>().space, dofs_vector_out );
